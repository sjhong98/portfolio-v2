...
export default function About() {
  ...
  return (
    ...
    >
      ...
      <p className="text-lg font-bold text-sub-light whitespace-pre-wrap break-keep">
        {mainContent.aboutTitle}
      </p>

      <div className="flex flex-col gap-3">
        {mainContent.about.map((item, index) => (
          <div key={index} className="flex gap-2">
            <p>·</p>
            <p className="sm:text-[16px] text-[14px] text-sub-light whitespace-pre-wrap break-keep leading-[24px]">
              {item}
            </p>
          </div>
        ))}
