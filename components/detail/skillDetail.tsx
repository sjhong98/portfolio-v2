...
export default function SkillDetail() {
    ...
    useEffect(() => {
        console.log(descriptionVisibleIndex)
        ...
        <div className='flex flex-col gap-12 relative'>
            ...
            <div className='absolute top-0 right-0 flex flex-col gap-1 items-end'>
                ...
                <div className='flex flex-row gap-2 items-center'>
                    ...
                    <p className='text-[12px] text-sub-light font-light'>Level 1</p>
                </div>
                <div className='flex flex-row gap-2 items-center'>
                    <div className='w-2 h-2 rounded-full' style={{ backgroundColor: '#539404' }} />
                    <p className='text-[12px] text-sub-light font-light'>Level 2</p>
                </div>
                <div className='flex flex-row gap-2 items-center'>
                    <div className='w-2 h-2 rounded-full' style={{ backgroundColor: '#294a02' }} />
                    <p className='text-[12px] text-sub-light font-light'>Level 3</p>
                </div>
            </div>

            {/* Skills by Category */}
            <div className="flex flex-col gap-6">
                {mainContent.skills.map((category, index) => (
                    <div key={category.category} className="flex flex-col gap-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {category.skills.map((skill) => (
                                <div
                                    key={skill.title}
                                    className='flex flex-col relative gap-1 h-fit bg-sub-darkest rounded-sm shadow-lg px-2 pt-2 cursor-pointer z-[100] transition-all duration-150 break-keep'
                                    style={{
                                        minWidth: selectedSkill === skill.title ? '200px' : '56px',
                                        width: selectedSkill === skill.title ? '240px' : `${(document.getElementById(skill.title) as HTMLElement)?.offsetWidth + 16}px`,
                                        maxWidth: selectedSkill === skill.title ? '240px' : 'none',
                                        minHeight: selectedSkill === skill.title ? '178px' : '48px',
                                ...
                                >
                                    ...
                                    <div className='absolute top-1.5 right-1.5'>
                                        <div className='w-1.5 h-1.5 rounded-full' style={{ backgroundColor: skill.level === 1 ? '#294a02' : skill.level === 2 ? '#539404' : '#a1f939' }} />
                                    </div>

                                    {
                                        descriptionVisibleIndex === skill.title && (
                                            <div
